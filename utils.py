...
try:
    ...
    keypoints_model = load_model('models/weights_keypoint.keras')
    emotion_model = load_model('models/weights_emotions.keras')
...
def process_image(image_path):
    try:
        ...
        # Detectar si es una imagen pequeña del dataset (típicamente 48x48)
        is_dataset_image = image.shape[0] < 100 or image.shape[1] < 100
        
        if is_dataset_image:
            # Si es imagen del dataset, redimensionarla a un tamaño más grande
            target_size = 400
            image = cv2.resize(image, (target_size, target_size), interpolation=cv2.INTER_CUBIC)
            print(f"Imagen del dataset redimensionada a {target_size}x{target_size}")
...
def write_emotion_on_image(image, emotion):
    try:
        ...
        emotion_es = EMOTION_TRANSLATION.get(emotion.lower(), emotion.upper())
        
        font = cv2.FONT_HERSHEY_SIMPLEX
        
        # Ajustar tamaño del texto según el tamaño de la imagen
        ...
        # Texto blanco con borde negro para máxima visibilidad
        text_color = (255, 255, 255)  # Blanco
        border_color = (0, 0, 0)       # Negro
        
        (w, h), _ = cv2.getTextSize(emotion_es, font, font_scale, thickness)
        
        x = int((image.shape[1] - w) / 2)
        y = int(image.shape[0] - 20 - padding)
        
        img_copy = image.copy()
        
        # Añadir fondo semi-transparente para mejor contraste
        ...
        cv2.putText(img_copy, emotion_es, (x, y), font, font_scale, 
